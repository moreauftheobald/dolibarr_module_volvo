...
$num = $reprise->fetchAll('', '', 0, 0, array(
		't.fk_lead' => $id
));
$lines = $reprise->lines;
...
if ($action == 'create_expertise') {
	foreach ( $expertise->value_array as $value ) {
		if (isset($_POST[$value]) && $_POST[$value] != - 1) {
			$expertise->$value = GETPOST($value);
		}
	}
	if (isset($_POST['date_exp_'])) {
		$expertise->date_exp = dol_mktime(0, 0, 0, GETPOST('date_exp_month'), GETPOST('date_exp_day'), GETPOST('date_exp_year'));
	}
	if (isset($_POST['date_essai_'])) {
		$expertise->date_essai = dol_mktime(0, 0, 0, GETPOST('date_essai_month'), GETPOST('date_essai_day'), GETPOST('date_essai_year'));
	}
	$res = $expertise->create($user);
	if ($res<0) {
		setEventMessages(null, $expertise->errors,'errors');
		header('Location:' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&action=create&repid=' . $repid . '&exp=' . $res);
	} else {
		header('Location:' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&action=&repid=' . $repid . '&exp=' . $res);
...
if ($action == 'update_expertise') {
	...
	$res = $expertise->update($user);
	if ($res == 1) {
		header('Location:' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&action=&repid=' . $repid . '&exp=' . $exp);
	} else {
		setEventMessages(null,$expertise->errors,'errors');
		header('Location:' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&action=update&repid=' . $repid . '&exp=' . $exp);
	}
}

if ($action == 'confirm_delete' && $confirm == 'yes' && $user->rights->volvo->gere_reprise) {
	$res = $expertise->delete($user);
	header('Location:' . dol_buildpath("/volvo/reprise/card.php", 1) . '?id=' . $object->id . '&action=&repid=' . $repid . '&exp=');
}
...
dol_fiche_head($head, $actif2, 'reprise', 0, dol_buildpath('/volvo/img/iron02.png', 1), 1);

if ($action == 'create') {
	print '<form name="createexpertise" action="' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&repid=' . $repid . '&exp=' . $exp . '" method="POST">';
	print '<input type="hidden" name="token" value="' . $_SESSION['newtoken'] . '">';
	print '<input type="hidden" name="action" value="create_expertise">';
	print '<input type="hidden" name="fk_rang" value="' . (count($expertise->liste_exp)==0?'1':'2'). '">';
	...
	print '</TR>';
	print '<TR>';
	print '<TD colspan="2"><b> L' . "'" . 'utilisateur vendeur' . $form->selectcontacts($object->fk_soc, '', 'fk_client_contact', 1) . '</TD>';
	...
	print '<form name="updateexpertise" action="' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&repid=' . $repid . '&exp=' . $exp . '" method="POST">';
	print '<input type="hidden" name="token" value="' . $_SESSION['newtoken'] . '">';
	...
	print '</TR>';
	print '<TR>';
	print '<TD colspan="2"><b> L' . "'" . 'utilisateur vendeur' . $form->selectcontacts($object->fk_soc, $expertise->fk_client_contact, 'fk_client_contact', 1) . '</TD>';
