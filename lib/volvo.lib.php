...
function stat_sell($filter=array()){
	...
	$array = array_merge_recursive($month,$arrayresult1,$arrayresult2,$arrayresult3,$arrayresult4,$arrayresult5);
	...
	foreach($month as  $m => $mois){
		...
		if($arrayresult1['nb_fact'][$m]>0){
			...
			$line['m_moy'] = $arrayresult4['margetheo'][$m]/$arrayresult1['nb_fact'][$m];
			$line['m_moy_r'] = $arrayresult4['margereal'][$m]/$arrayresult1['nb_fact'][$m];
			$line['m_moy_e'] = ($arrayresult4['margereal'][$m]-$arrayresult4['margetheo'][$m])/$arrayresult1['nb_fact'][$m];
		}
		$line['vcm'] = $arrayresult2['vcm'][$m];
		$totvcm+= $arrayresult2['vcm'][$m];
		$line['dfol'] = $arrayresult2['dfol'][$m];
		$totdfol+= $arrayresult2['dfol'][$m];
		$line['dded'] = $arrayresult2['dded'][$m];
		$totdded+= $arrayresult2['dded'][$m];
		$line['lixbail'] = $arrayresult2['lixbail'][$m];
		$totlix+= $arrayresult2['lixbail'][$m];
		$line['vfs'] = $arrayresult2['vfs'][$m];
		$totvfs+= $arrayresult2['vfs'][$m];
		$line['m_tot'] = $arrayresult4['margetheo'][$m];
		$totmgt+= $arrayresult4['margetheo'][$m];
		$line['m_tot_r'] = $arrayresult4['margereal'][$m];
		$totmgr+= $arrayresult4['margereal'][$m];
		$line['m_tot_e'] = $arrayresult4['margereal'][$m]-$arrayresult4['margetheo'][$m];
		$result[$m] = $line;
		$var = !$var;
	}
	$line=array();
	$line['class'] = ' class="liste_titre"';
	$line['class_td'] = ' class="liste_titre"';
	$line['mois'] = 'Total';
	$line['nb_facture'] = $totfact;
	$line['nb_portfeuille'] = $totport;
	$line['ca_total'] =$totcato;
	$line['ca_volvo'] = $totcavo;
	$line['nb_trt'] = $tottrt;
	$line['nb_port'] = $totprt;
	if($totfact>0){
